...
class RestaurantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Restaurant; fields = "__all__"
        read_only_fields = ["owner", "is_approved", "average_rating", "created_at", "updated_at"]
class CategorySerializer(serializers.ModelSerializer):
    class Meta:
        model = Category; fields = "__all__"; read_only_fields = ["created_at", "updated_at"]
class MenuItemSerializer(serializers.ModelSerializer):
    class Meta:
        model = MenuItem; fields = "__all__"; read_only_fields = ["restaurant", "created_at", "updated_at"]
class AddressSerializer(serializers.ModelSerializer):
    class Meta:
        model = Address; fields = "__all__"; read_only_fields = ["user", "created_at", "updated_at"]
class CartItemSerializer(serializers.ModelSerializer):
    menu_item_detail = MenuItemSerializer(source="menu_item", read_only=True)
    class Meta:
        model = CartItem; fields = ["id", "menu_item", "menu_item_detail", "quantity", "created_at", "updated_at"]
        read_only_fields = ["id", "created_at", "updated_at"]
    def validate_quantity(self, value):
        if value < 1 or value > 99: raise serializers.ValidationError("Quantity must be between 1 and 99.")
        return value
...
class WishlistSerializer(serializers.ModelSerializer):
    menu_item_detail = MenuItemSerializer(source="menu_item", read_only=True)
    class Meta:
        model = Wishlist; fields = "__all__"; read_only_fields = ["user", "created_at", "updated_at"]
class CouponSerializer(serializers.ModelSerializer):
    class Meta:
        model = Coupon; fields = "__all__"; read_only_fields = ["usage_count", "created_at", "updated_at"]
...
class PaymentSerializer(serializers.ModelSerializer):
    class Meta:
        model = Payment; fields = "__all__"; read_only_fields = ["order", "amount", "created_at", "updated_at"]
class DeliverySerializer(serializers.ModelSerializer):
    class Meta:
        model = DeliveryAssignment; fields = "__all__"
        read_only_fields = ["order", "created_at", "updated_at", "pickup_at", "delivered_at"]
    def validate_partner(self, partner):
        request = self.context.get("request")
        if partner and partner.role != User.Role.DELIVERY:
            raise serializers.ValidationError("Partner must be a delivery user.")
        if request and request.user.role != User.Role.ADMIN and self.instance and partner and partner.id != self.instance.partner_id:
            raise serializers.ValidationError("Only an admin can reassign a delivery partner.")
        return partner
...
class NotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Notification; fields = "__all__"; read_only_fields = ["user", "created_at", "updated_at"]
class ReviewSerializer(serializers.ModelSerializer):
    class Meta:
        model = Review; fields = "__all__"; read_only_fields = ["customer", "restaurant", "created_at", "updated_at"]
    def validate_order(self, order):
        request = self.context.get("request")
        if not request or order.customer_id != request.user.id: raise serializers.ValidationError("You can only review your own orders.")
        if order.status != Order.Status.DELIVERED: raise serializers.ValidationError("You can review an order only after delivery.")
        return order
    def validate(self, attrs):
        order = attrs.get("order")
        if order and Review.objects.filter(order=order).exists() and not self.instance: raise serializers.ValidationError({"order": "This order has already been reviewed."})
        if order: attrs["restaurant"] = order.restaurant
        return attrs
class AnalyticsEventSerializer(serializers.ModelSerializer):
    class Meta:
        model = AnalyticsEvent; fields = "__all__"; read_only_fields = ["user", "created_at", "updated_at"]
...
class CheckoutSerializer(serializers.Serializer):
    ...
    def create(self, data):
        ...
        if unavailable: raise serializers.ValidationError({"cart": f"Unavailable items: {', '.join(unavailable)}"})
        ...
        subtotal = sum((i.menu_item.price * i.quantity for i in items), Decimal("0")); discount = Decimal("0"); coupon = None; now = timezone.now()
        if data.get("coupon_code"):
            coupon = Coupon.objects.select_for_update().filter(code__iexact=data["coupon_code"], is_active=True, starts_at__lte=now, ends_at__gt=now).first()
            ...
            if coupon.usage_limit is not None and coupon.usage_count >= coupon.usage_limit: raise serializers.ValidationError({"coupon_code": "Coupon usage limit has been reached."})
            discount = min(coupon.discount_amount or (subtotal * coupon.discount_percent / 100), subtotal)
            coupon.usage_count += 1; coupon.save(update_fields=["usage_count", "updated_at"])
        ...
        cart.items.all().delete(); cart.restaurant = None; cart.save(update_fields=["restaurant", "updated_at"])
