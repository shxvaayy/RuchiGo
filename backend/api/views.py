...
class AuthViewSet(viewsets.GenericViewSet):
    ...
    def logout(self, request):
        ...
        except Exception: return Response({"detail": "A valid refresh token is required."}, status=400)
...
class CategoryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs=super().get_queryset(); return qs.filter(is_active=True) if self.action in ["list","retrieve"] else qs
...
class MenuItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs=super().get_queryset()
        if self.action in ["list","retrieve"]: return qs.filter(is_available=True,restaurant__is_approved=True,restaurant__is_open=True,category__is_active=True)
        if self.request.user.is_superuser or self.request.user.role==User.Role.ADMIN: return qs
        return qs.filter(restaurant__owner=self.request.user)

class OwnedViewSet(viewsets.ModelViewSet):
    permission_classes=[permissions.IsAuthenticated]; owner_field="user"
...
class CartViewSet(viewsets.ViewSet):
    ...
    def list(self, request): cart,_=Cart.objects.get_or_create(user=request.user); return Response(CartSerializer(cart).data)
    @action(detail=False, methods=["post"])
    def items(self, request):
        cart,_=Cart.objects.get_or_create(user=request.user)
        try: menu_id=int(request.data.get("menu_item")); quantity=int(request.data.get("quantity",1))
        except (TypeError,ValueError): return Response({"detail":"menu_item and quantity must be valid integers."},status=400)
        if quantity<1 or quantity>99: return Response({"detail":"Quantity must be between 1 and 99."},status=400)
        menu=MenuItem.objects.filter(pk=menu_id,is_available=True,restaurant__is_approved=True,restaurant__is_open=True).first()
        ...
        cart.restaurant=menu.restaurant; cart.save(update_fields=["restaurant","updated_at"])
        item,created=CartItem.objects.get_or_create(cart=cart,menu_item=menu,defaults={"quantity":quantity})
    ...
    def item(self, request, item_id=None):
        ...
        if request.method=="DELETE": item.delete()
            ...
            except (TypeError,ValueError): return Response({"detail":"Quantity must be a valid integer."},status=400)
            if quantity<1 or quantity>99: return Response({"detail":"Quantity must be between 1 and 99."},status=400)
            item.quantity=quantity; item.save(update_fields=["quantity","updated_at"])
        if not cart.items.exists(): cart.restaurant=None; cart.save(update_fields=["restaurant","updated_at"])
        return Response(CartSerializer(cart).data)
    @action(detail=False, methods=["post"])
    def checkout(self, request):
        serializer=CheckoutSerializer(data=request.data,context={"request":request}); serializer.is_valid(raise_exception=True); return Response(OrderSerializer(serializer.save()).data,status=201)

class OrderViewSet(viewsets.ReadOnlyModelViewSet):
    serializer_class=OrderSerializer; filterset_fields=["status","restaurant"]
    def get_queryset(self):
        user=self.request.user; qs=Order.objects.select_related("restaurant","customer","delivery_address","payment","delivery").prefetch_related("items")
    ...
    def status(self, request, pk=None):
        ...
        transitions={Order.Status.PENDING:{Order.Status.CONFIRMED,Order.Status.CANCELLED},Order.Status.CONFIRMED:{Order.Status.PREPARING,Order.Status.CANCELLED},Order.Status.PREPARING:{Order.Status.READY,Order.Status.CANCELLED},Order.Status.READY:{Order.Status.OUT,Order.Status.CANCELLED},Order.Status.OUT:{Order.Status.DELIVERED},Order.Status.DELIVERED:set(),Order.Status.CANCELLED:set()}
        role_allowed={User.Role.RESTAURANT:{Order.Status.CONFIRMED,Order.Status.PREPARING,Order.Status.READY,Order.Status.CANCELLED},User.Role.DELIVERY:{Order.Status.OUT,Order.Status.DELIVERED},User.Role.ADMIN:{value for value,_ in Order.Status.choices}}
        if request.user.role not in role_allowed or new not in role_allowed[request.user.role]: return Response({"detail":"Status transition not allowed."},status=403)
        if new not in transitions.get(order.status,set()): return Response({"detail":f"Cannot change order from {order.status} to {new}."},status=400)
        order.status=new; order.save(update_fields=["status","updated_at"])
        if new==Order.Status.OUT: DeliveryAssignment.objects.filter(order=order).update(pickup_at=timezone.now())
        if new==Order.Status.DELIVERED: DeliveryAssignment.objects.filter(order=order).update(delivered_at=timezone.now())
...
class UserManagementViewSet(viewsets.ModelViewSet): queryset=User.objects.all().order_by("-created_at"); serializer_class=UserSerializer; permission_classes=[IsAdmin]
class CouponViewSet(viewsets.ModelViewSet): queryset=Coupon.objects.all(); serializer_class=CouponSerializer; permission_classes=[IsAdmin]; lookup_field="code"
...
class DeliveryViewSet(viewsets.ModelViewSet):
    queryset=DeliveryAssignment.objects.select_related("order","partner"); serializer_class=DeliverySerializer
    def get_permissions(self):
        if self.action in ["create","destroy"]: return [IsAdmin()]
        if self.action in ["update","partial_update"]: return [permissions.IsAuthenticated()]
        return [permissions.IsAuthenticated()]
    def get_queryset(self):
        user=self.request.user
        if user.is_superuser or user.role==User.Role.ADMIN: return super().get_queryset()
        return super().get_queryset().filter(partner=user)
    def perform_create(self, serializer):
        order=serializer.validated_data["order"]
        if DeliveryAssignment.objects.filter(order=order).exists():
            raise serializers.ValidationError({"order":"This order already has a delivery assignment."})
        serializer.save()
    def perform_update(self, serializer):
        if self.request.user.role != User.Role.ADMIN and serializer.validated_data.get("partner") not in [None, self.request.user]:
            raise permissions.PermissionDenied("You can only update your own delivery assignment.")
        serializer.save()
