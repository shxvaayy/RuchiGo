...
class RestaurantViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ["city", "is_open", "is_approved"]
...
class MenuItemViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ["restaurant", "category", "is_available", "is_vegetarian"]
    ...
    def get_queryset(self):
        qs = super().get_queryset()
        if self.action in ["list", "retrieve"]:
            return qs.filter(is_available=True, restaurant__is_approved=True, restaurant__is_open=True, category__is_active=True)
...
class WishlistViewSet(OwnedViewSet):
    queryset = Wishlist.objects.select_related("menu_item")
...
class ReviewViewSet(OwnedViewSet):
    queryset = Review.objects.select_related("restaurant")
...
class CartViewSet(viewsets.ViewSet):
    ...
    def items(self, request):
        cart, _ = Cart.objects.get_or_create(user=request.user)
        try:
            menu_id = int(request.data.get("menu_item"))
            quantity = int(request.data.get("quantity", 1))
        except (TypeError, ValueError):
            return Response({"detail": "menu_item and quantity must be valid integers."}, status=400)
        if quantity < 1 or quantity > 99:
            return Response({"detail": "Quantity must be between 1 and 99."}, status=400)
        ...
        if cart.restaurant and cart.restaurant_id != menu.restaurant_id:
            return Response({"detail": "Cart can contain one restaurant only."}, status=400)
        cart.restaurant = menu.restaurant
    ...
    def checkout(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        return Response(OrderSerializer(serializer.save()).data, status=201)
...
class OrderViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = Order.objects.select_related("restaurant", "customer", "delivery_address", "payment", "delivery").prefetch_related("items")
    ...
    def status(self, request, pk=None):
        ...
        transitions = {
            ...
            Order.Status.READY: {Order.Status.OUT, Order.Status.CANCELLED},
            Order.Status.OUT: {Order.Status.DELIVERED},
            Order.Status.DELIVERED: set(),
            Order.Status.CANCELLED: set(),
        }
        role_allowed = {
            User.Role.RESTAURANT: {Order.Status.CONFIRMED, Order.Status.PREPARING, Order.Status.READY, Order.Status.CANCELLED},
            User.Role.DELIVERY: {Order.Status.OUT, Order.Status.DELIVERED},
            User.Role.ADMIN: {value for value, _ in Order.Status.choices},
        }
        if request.user.role not in role_allowed or new not in role_allowed[request.user.role]:
            return Response({"detail": "Status transition not allowed."}, status=403)
        if new not in transitions.get(order.status, set()):
            return Response({"detail": f"Cannot change order from {order.status} to {new}."}, status=400)
        order.status = new
        order.save(update_fields=["status", "updated_at"])
        if new == Order.Status.OUT:
            DeliveryAssignment.objects.filter(order=order).update(pickup_at=timezone.now())
...
class PaymentViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        qs = Payment.objects.select_related("order", "order__restaurant")
        if user.is_superuser or user.role == User.Role.ADMIN:
            return qs
        if user.role == User.Role.RESTAURANT:
...
class OfferViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return super().get_queryset() if self.request.user.is_superuser else super().get_queryset().filter(restaurant__owner=self.request.user)
...
class DeliveryViewSet(viewsets.ModelViewSet):
    queryset = DeliveryAssignment.objects.select_related("order")
...
class AnalyticsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        return Response({
            ...
            "orders": Order.objects.values("status").annotate(count=Count("id"), revenue=Sum("total")),
            "restaurants": Restaurant.objects.filter(is_approved=True).count(),
        })
